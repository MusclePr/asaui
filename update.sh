...
#    cluster/.env <--- cluster.template/defaults/template.env から更新します
#    cluster/common.env <--- cluster.template/defaults/common.env から更新します
#    cluster/web/dynamicconfig.ini <--- cluster.template/web/dynamicconfig.ini から更新します

update_user_file "cluster.template/defaults/template.env" "cluster/.env"
update_user_file "cluster.template/common.env" "cluster/common.env"
update_user_file "cluster.template/web/dynamicconfig.ini" "cluster/web/dynamicconfig.ini"

# ファイルが無ければ作成します
if [ ! -f "cluster/.env" ]; then
    echo "Creating empty .env file..."
    cp -av "cluster.template/defaults/template.env" "cluster/.env"
fi

if [ ! -f "cluster/common.env" ]; then
    echo "Creating empty common.env file..."
    cp -av "cluster.template/defaults/common.env" "cluster/common.env"
fi

# マップの数は、cluster/.env ファイルの ASA0_SERVER_MAP から ASA9_SERVER_MAP までの変数の有効な定義の有無で決まります
echo "services:" > "cluster/compose.override.yml"
for i in {1..9}; do
    # ^ASA${i}_SERVER_MAP=\w+ で始まる行が cluster/.env ファイルに存在すれば、そのマップは有効とみなします
    if grep -q "^ASA${i}_SERVER_MAP=\w\+" "cluster/.env"; then
        add_asa_section "cluster/compose.override.yml" "$i"
    fi
done
